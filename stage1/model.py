...
def fit_state_dict_to_model(state_dict, model):
    ...
    if model_has_module and not state_has_module:
        return add_module_prefix(state_dict)
    if not model_has_module and state_has_module:
        return strip_module_prefix(state_dict)
    return state_dict


def load_checkpoint(path, device, unsafe_load=False):
    path = os.fspath(path)
    if not os.path.isfile(path):
        raise FileNotFoundError(f"checkpoint not found: {path}")
    try:
        ckpt = torch.load(path, map_location=device, weights_only=not unsafe_load)
    except TypeError:
        ckpt = torch.load(path, map_location=device)
    return unwrap_state_dict(ckpt)


def load_weights(model, checkpoint, device, unsafe_load=False):
    state_dict = load_checkpoint(checkpoint, device, unsafe_load)
    model.load_state_dict(fit_state_dict_to_model(state_dict, model), strict=True)
